...
# Batch script to calculate dnds for each orthogroup focusing on one species
...
#$ -N CodemlVespaBranchSite

# put error messages and output in the log directory
#$ -e logs/$JOB_NAME
#$ -o logs/$JOB_NAME
...
mkdir -p /SAN/ugi/VespaCrabro/input/config-files/BS/${species}/
mkdir -p /SAN/ugi/VespaCrabro/result/calculating-dnds/branch-site-model/${species}/
