...
# Obtain longest transcript for  each gene in Vespa vulgaris
...
wget https://ftp.ncbi.nlm.nih.gov/genomes/all/GCA/014/466/185/GCA_014466185.1_ASM1446618v1/GCA_014466185.1_ASM1446618v1_genomic.gff.gz

mv GCA_014466185.1_ASM1446618v1_genomic.gff.gz  V_vulgaris.gff.gz

thisspecies="V_vulgaris"
...
# 81843 instances of gene names 
zcat ${thisspecies}.gff.gz | awk '$3=="CDS"{gsub("ID=cds.+JACSEA\\|", "", $9); gsub(";.*", "", $9); gsub("\\.t[[:digit:]]", "", $9) ; print $9}' | wc -l

# 81843 instances of protein names
...
zcat ${thisspecies}.gff.gz | awk '$3=="CDS"{gsub("ID=cds.+JACSEA\\|", "", $9); gsub(";.*", "", $9); gsub("\\.t[[:digit:]]", "", $9) ; print $9}' > ${thisspecies}-gene-names
...
# this protein fasta has 15512 longest isoforms, with headers containing just gene id (i.e. >NP_001291509.1)
