...
# Obtain longest transcript for  each gene in Vespula germanica
...
wget https://ftp.ncbi.nlm.nih.gov/genomes/all/GCA/014/466/195/GCA_014466195.1_ASM1446619v1/GCA_014466195.1_ASM1446619v1_genomic.gff.gz

mv GCA_014466195.1_ASM1446619v1_genomic.gff.gz V_germanica.gff.gz

thisspecies="V_germanica"
...
# 87246 instances of gene names 
zcat ${thisspecies}.gff.gz | awk '$3=="CDS"{gsub(".+WGS:JACSDZ\\|", "", $9); gsub(";.*", "", $9) ; print $9}' | wc -l

# 87246 instances of protein names
...
zcat ${thisspecies}.gff.gz | awk '$3=="CDS"{gsub(".+WGS:JACSDZ\\|", "", $9); gsub(";.*", "", $9) ; print $9}' > ${thisspecies}-gene-names
...
# this protein fasta has 17187 longest isoforms, with headers containing just gene id (i.e. >NP_001291509.1)
